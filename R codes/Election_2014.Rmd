...
```{r import_csv}
library(readxl)
Legislative_Election <- read_excel("../Data/Party in Legislative Election.xls")
getwd()
```
...
``` {r total_votes}
totalvotes <- read_excel("../Data/Total Votes percandidates.xlsX")
glimpse(totalvotes)
```
...
*Not sure what's going on here - @Fahrozi you'll need to debug this*

```{r proportion_of_votes}
totalvotes <- totalvotes %>% mutate(Votes=as.numeric(Votes))
P <- totalvotes %>%
