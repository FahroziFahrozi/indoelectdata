...
```{r import-csv}
library(readxl)
Party_in_Legislative_Election <- read_excel("../Data/Party in Legislative Election.xlsx")
```
...
library(broom)
library(readxl)
glimpse(Party_in_Legislative_Election) %>% head %>% kable
```
...
```{r total-votes}
totalvotes <- read_excel("../Data/totalvotes.xlsx")
glimpse(totalvotes)
```
...
```{r Totalseats}
Totalseats <- read_excel("../Data/Totalseats.xlsx")
glimpse(Totalseats)
```
```{r total_votes_for_parties}
Totalseats <- Totalseats %>% mutate(Totseats=as.numeric(Totalseats))
