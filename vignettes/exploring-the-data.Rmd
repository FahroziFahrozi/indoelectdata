...
postcode <- readOGR("../Shapefile/INDONESIA_PROP.shp")
postcode@data$ID <- rownames(postcode@data)
totalvotes <- read.csv(here::here("data-raw/Total Votes percandidates.csv"))
map.winners <- merge(postcode, totalvotes, by.x="Propinsi", by.y="Electorate")
map.winners@data$ID <- rownames(map.winners@data)
#groupping
map.winners$PartyNm <- map.winners$PartyNm
...
ggplot(data = map.winners) +
  ...
  scale_fill_manual(name= "Election party", values=partycolours) +
