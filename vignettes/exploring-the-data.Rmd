...
Legislative_Election <- read_excel(here::here("vignettes/Data/Party in Legislative Election.xls"))
usethis::use_data(Legislative_Election, overwrite = TRUE)
```
...
who_won %>%
  kable()
usethis::use_data(who_won, overwrite = TRUE )
```
...
``` {r total-votes}
totalvotes <- read_excel(here::here("vignettes/Data/Total Votes percandidates.xlsx"))
usethis::use_data(totalvotes, overwrite = TRUE)
```
...
total_votes_for_parties %>%
  kable
usethis::use_data(total_votes_for_parties, overwrite = TRUE)
```
...
``` {r totalseats}
Totalseats <- read_excel(here::here("vignettes/Data/Totalseats.xlsx"))
usethis::use_data(Totalseats, overwrite = TRUE)
```
...
Total_seats_for_parties  %>%
  kable
usethis::use_data(Total_seats_for_parties, overwrite = TRUE)
```
...
Total_seats_for_provinces %>%
  kable
usethis::use_data(Total_seats_for_provinces, overwrite = TRUE)
```
...
total_votes_for_candidate %>%
  head %>% 
  kable
usethis::use_data(total_votes_for_candidate, overwrite = TRUE)
```
...
who_won_most_votes_prop %>%
  head %>%
  kable
usethis::use_data(who_won_most_votes_prop, overwrite = TRUE)
```
...
BM: I'm not 100% sure what you want this plot to look like. Can you explain more?
...
usethis::use_data(province_plots, overwrite = TRUE)
```
